  ...
  useEffect(() => {
    ...
    const checkUnreadActivity = async () => {
      ...
      try {
        ...
        setHasUnreadActivity(
          data.hasUnread || data.count > 0
        );
      } catch (error) {
        console.error(
          "Failed to fetch unread activity status",
          error
        );
      }
    };
    ...
    try {
      ...
      if (notificationsEnabled) {
        if (Capacitor.isNativePlatform()) {
          ...
          alert(
            "Notifications paused in MoBoxd. You can re-enable them anytime."
          );
        } else {
          alert(
            "To disable notifications, please click the lock icon in your browser URL bar and block notifications."
          );
        }
      ...
      // IMPORTANT:
      // Do NOT call PushNotifications.register()
      // directly here.
      //
      // pushHelper handles:
      // permission -> listener -> register -> save token

      const success =
        await subscribeToPushNotifications(user.token);
      ...
      if (success) {
        ...
        if (Capacitor.isNativePlatform()) {
          alert(
            "Notification permission was denied. Please enable notifications for MoBoxd in your phone Settings."
          );
        } else {
          alert(
        ...
        <>
          ...
          <div className="md:hidden fixed top-16 right-0 w-full sm:w-64 bg-[#1A1A21] border-b sm:border border-[#2A2A35] shadow-2xl z-40 flex flex-col py-2 px-4 sm:mr-4 sm:mt-2 sm:rounded-xl">
            ...
            >
                ...
                <i
                  className={`bi ${
                    notificationsEnabled
                      ? "bi-bell-fill text-moboxd-accent"
                      : "bi-bell-slash text-moboxd-muted"
                  } text-xl`}
                ></i>
              ...
              >
                <div
                  className={`w-4 h-4 bg-black rounded-full absolute top-0.5 transition-all ${
                    notificationsEnabled
                      ? "right-0.5"
                      : "left-0.5"
                  }`}
